...
import pandas as pd
import numpy as np
ener = pd.read_excel('Energy Indicators.xls')
ener = ener[16:243]
ener = ener.drop(ener.columns[[0, 1]], axis=1)
ener.rename(columns={'Environmental Indicators: Energy': 'Country','Unnamed: 3':'Energy Supply','Unnamed: 4':'Energy Supply per Capita','Unnamed: 5':'% Renewable'}, inplace=True)
ener.replace('...', np.nan,inplace = True)
ener['Energy Supply'] *= 1000000
def remove_digit(data):
    newData = ''.join([i for i in data if not i.isdigit()])
    i = newData.find('(')
    if i>-1: newData = newData[:i]
    return newData.strip()
ener['Country'] = ener['Country'].apply(remove_digit)
di = {"Republic of Korea": "South Korea",
"United States of America": "United States",
"United Kingdom of Great Britain and Northern Ireland": "United Kingdom",
"China, Hong Kong Special Administrative Region": "Hong Kong"}
ener.replace({"Country": di},inplace = True)
ener.head()
...
df = pd.merge(pd.merge(ener, GDP, on='Country'), ScimEn, on='Country')

# Les donnees compris entre 2006-2015 
...
def answer_two():
    union = pd.merge(pd.merge(ener, GDP, on='Country', how='outer'), ScimEn, on='Country', how='outer')
    intersection = pd.merge(pd.merge(ener, GDP, on='Country'), ScimEn, on='Country')
...
def answer_six():
    Top15 = answer_one()
    cont = Top15.sort_values(by='% Renewable', ascending=False).iloc[0]
    return (cont.name, cont['% Renewable'])
answer_six()
...
def answer_seven():
    ...
    cont = Top15.sort_values(by='Citation_ratio', ascending=False).iloc[0]
    return cont.name, cont['Citation_ratio']
answer_seven()
...
def answer_eleven():
    ...
    ContinentDict  = {'China':'Asia', 
                  ...
                  'Australia':'Australia', 
                  'Brazil':'South America'}
    groupe = pd.DataFrame(columns = ['size', 'sum', 'mean', 'std'])
    Top15['Estimate Population'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    for group, frame in Top15.groupby(ContinentDict):
        groupe.loc[group] = [len(frame), frame['Estimate Population'].sum(),frame['Estimate Population'].mean(),frame['Estimate Population'].std()]
    return groupe
answer_eleven()
